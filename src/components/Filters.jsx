...
const Filters = () => {
  ...
  return (
    <div className="w-full border-2 border-gray-700 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-start sm:justify-between p-4 gap-4 flex-wrap">
      <Search />

      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        
        <form
          className="relative flex items-center font-nunito gap-2"
          onSubmit={handleCurrencySubmit}
        >
          <label
            htmlFor="currency"
            className="flex justify-center items-center font-bold text-sm sm:text-base"
          >
            currency:
          </label>
          <input
            ...
            ref={currencyRef}
            placeholder="usd"
            className="w-16 rounded bg-gray-700 placeholder:text-gray-500 pl-2 outline-0 border border-transparent focus:border-cyan-400 leading-4"
            required
          />
        ...
        <label className="flex items-center text-sm sm:text-base font-bold relative">
          <span className="mr-2">sort by:</span>
          <select
            name="sortby"
            className="rounded bg-gray-700 text-sm sm:text-base w-full sm:w-auto pl-2 pr-10 py-0.5 leading-4 capitalize focus:outline-0"
            onChange={handleSort}
          >
          ...
          <img
            src={selectIcon}
            alt="submit"
            className="w-[1rem] h-auto absolute right-1 top-[50%] translate-y-[-50%] pointer-events-none"
          />
        </label>
