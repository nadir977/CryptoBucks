...
const Trending = () => {
  ...
  const { trendData, showGraph, setShowGraph, getCoinDetails, resetCryptoContext ,getTrendData} = useContext(CryptoContext);
useEffect(()=>{
getTrendData()
},[])
  ...
  return (
    ...
    <section className="w-[80%] h-full flex flex-col mt-16 mb-24 relative">
      <button className="w-[2rem] ml-4 hover:scale-110 transition-all transition-ease absolute right-4 -top-10" onClick={resetCryptoContext}>
  <svg
  ...
  >
    ...
    />
      ...
      <div className="w-full grid grid-cols-2 gap-5 p-3  border border-gray-700 rounded">
        {trendData?.map((coins, index) => {
          const coin = coins.item
          return <div key={index} onClick={() => {getCoinDetails(coin.id);setShowGraph(true)}} className="w-[80%] bg-gray-800 rounded-lg p-4 relative cursor-pointer hover:bg-gray-600 hover:bg-opacity-40 last:mb-0">
            <h3 className="text-base flex items-center my-0.5 ">
            <span className="text-gray-500 capitalize ">name:&nbsp;</span>
            <span className="text-cyan-400">{coin.name}</span>
          </h3>
          ...
          <h3 className="text-base flex items-center my-0.5 ">
            ...
            <span className="text-cyan-400">{coin.market_cap_rank}</span>
          </h3>
          ...
          <h3 className="text-base flex items-center my-0.5 ">
            ...
            <span className="text-cyan-400">{coin.score}</span>
          </h3>

          <img
            src={coin.large}
            alt={coin.name}
            className="w-[140px] h-auto rounded-full absolute top-2/4 -right-12 -translate-y-2/4"
          />
          </div>
        })}
      </div>
